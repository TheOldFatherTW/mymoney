(function () {
  ...
  var waitBar = document.getElementById("invite-wait-bar");
  ...
  var LOCK_STORE = "mymoney.gateLock";
  var typed = "";
  var need = 4;
  var busy = false;
  var lockedUntil = 0;
  ...
  function doorKey() {
    return window.MYMONEY_URL_KEY || (window.FamiGate && window.FamiGate.currentKey()) || "";
  }
  ...
  function startWait() {
    ...
    if (waitBar && window.MoneyMark) window.MoneyMark.mountBar(waitBar);
  ...
  function scheduleReconnect() {
    ...
    retryTimer = setTimeout(function () {
      retryTimer = 0;
      if (window.MoneyDoor && window.MoneyDoor.refreshOrigin) {
        window.MoneyDoor.refreshOrigin().then(probePublic);
      } else {
        probePublic();
      }
    }, 12000);
  ...
  function goHome(token) {
    try { sessionStorage.setItem("mymoney.gateOk", "1"); } catch (e) {}
    window.FamiGate.savePersonal(token);
    window.MYMONEY_VIEW_KEY = token;
    window.MYMONEY_NEED_GATE = false;
    document.documentElement.classList.remove("need-gate");
    document.documentElement.classList.add("gate-ok");
    startWait();
    if (window.MoneyDoor && window.MoneyDoor.enterAfterGate) {
      window.MoneyDoor.enterAfterGate(token);
    }
  }
  ...
  function submit(isRetry) {
    ...
    busy = true;
    paintLock();
    window.FamiGate.api("/api/gate", key, {
      method: "POST",
      ...
      var j = x && x.j;
      if (j && j.ok && j.token && window.FamiGate.KEY_RE.test(j.token)) {
        goHome(j.token);
        return;
      }
      if (j && j.error === "locked") {
      ...
      if (!isRetry && window.MoneyDoor && window.MoneyDoor.refreshOrigin) {
        window.MoneyDoor.refreshOrigin().then(function () { submit(true); });
        return;
      }
  ...
  function probePublic() {
    if (!window.FamiGate.origin() && location.protocol === "https:") {
      if (statusEl) statusEl.textContent = "維護中,請5分鐘後再試";
      scheduleReconnect();
      return;
    }
    window.FamiGate.api("/api/public", "", { timeout: 8000 }).then(function (x) {
  ...
  function openGate() {
    window.MYMONEY_NEED_GATE = true;
    try { sessionStorage.removeItem("mymoney.gateOk"); } catch (e) {}
  ...
  document.addEventListener("keydown", function (ev) {
    if (!window.MYMONEY_NEED_GATE || busy || lockLeft() > 0) return;
    if (ev.key === "Backspace") {
      ev.preventDefault();
      popDigit();
      return;
  ...
  window.MoneyGate = { open: openGate };
  boot();
})();
